...
- Stripe Cash App Pay: configure `STRIPE_SECRET_KEY` as a Cloudflare Worker secret (`nub exec wrangler secret put STRIPE_SECRET_KEY`), and `STRIPE_SHIPPING_RATE_ID` plus `PUBLIC_APP_URL` as Worker vars and in local `.dev.vars`. Create a USD shipping rate in the Stripe Dashboard and use an HTTPS production URL. Stripe-hosted Checkout is used; payment details never enter the app. Cash App Pay requires a US-based Stripe business and USD-priced products. Current catalogue prices are illustrative; set business-ready prices in `src/lib/snacks.ts`. Stripe API credentials are only read by `src/server/payments/stripe.functions.ts`.
...
- Gotchas: `src/lib/snacks.ts` is the price authority for Stripe sessions; never accept price values from the browser. Cash App Pay only accepts USD and US shipping addresses. The app has no inventory service, so update `available` in the catalogue manually.
